...
текущей версии), а не production-конфиг конкретной машины. Его производят из
tracked-шаблона
`deployment\SearchEngineServicePortable\source-data\Settings.json` скриптом
`scripts\Prepare-YearBasedReleaseSettings.ps1` только внутри package staging.
По умолчанию в `config.year` записывается календарный год выпуска, а последняя
цифра `config.asio_port` заменяется последней цифрой года; остальные цифры порта
сохраняются. Например, порт `15006` станет `15007` для 2027 года и `15000` для
2030 года. Для воспроизводимого пакета сборщик принимает `-Year <год>`.

Генератор не запускается обычной Debug-сборкой, установленной службой или
установщиком на целевом компьютере и не изменяет исходный tracked-шаблон.
Активный `<data-dir>\Settings.json` создаётся workflow установки/настройки. При
update/reinstall установщик начинает с шаблона нового релиза и рекурсивно
импортирует существующий installed Settings: старые значения (включая год и
порт) и unknown-поля сохраняются, новые поля релиза получают defaults из
template, если их не было в старом конфиге; явные значения диалога имеют
