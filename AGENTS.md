...
## Синхронизация команд и ошибок протокола

Сервер и клиент находятся в независимых репозиториях, но `COMMAND`,
`ErrorCode`, capability bits, версии протокола и бинарные response payload
образуют единый wire-контракт.

- Добавление, удаление, переименование, перестановка или изменение числового
  значения команды/ошибки, capability bit, версии либо структуры wire-payload
  всегда выполнять как двухрепозиторное изменение. До включения нового
  поведения проверить и при необходимости обновить обе стороны в одном этапе.
- Числовые значения wire-enum задавать явно и сохранять стабильными. Не
  вставлять элементы так, чтобы перенумеровывались существующие команды.
  Обратную совместимость обеспечивать negotiation/capability и legacy fallback.
- На обеих сторонах синхронно проверять enum, текстовые mappings,
  сериализацию/парсинг, `sizeof`/ordinal `static_assert` и профильные
  тесты/self-tests. После изменения собрать и проверить затронутые цели обоих
  проектов.
- Коммиты делать отдельно в каждом репозитории и в отчёте указывать оба hash.
  Если вторую сторону нельзя обновить или проверить, несовместимое wire-поведение
  не включать: сохранить fallback и явно доложить ограничение.
- Начало использования серверным обработчиком уже объявленного `ErrorCode` не
  требует формальной no-op правки клиента. Но обязательно подтвердить, что
  клиент уже знает числовой код, корректно читает его и имеет подходящий
  UI/fallback для соответствующей команды.
