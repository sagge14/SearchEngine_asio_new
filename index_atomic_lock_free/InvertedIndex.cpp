        ...
        std::shared_ptr<std::promise<void>> promise)
{
    try
    {
        ...
        const std::wstring& fullPath = docPaths.pathById(fileId);
        ...
        batch.chunkMap.reserve(freqWordFile.size());

        // === 1. Собираем массив слов и соответствующих count ===
        std::vector<std::string> words;
        words.reserve(freqWordFile.size());

        std::vector<uint32_t> counts;
        counts.reserve(freqWordFile.size());
    ...
    // === 2. ОДИН batch-вызов для всех слов документа ===
        std::vector<uint32_t> wids = wordIds.bulkGetIds(words);

        // --- формируем fileRefs и chunkMap ---
        batch.fileRefs.reserve(wids.size());

        // === 1. Готовим fileRefs и chunkMap в worker-потоке ===
        std::vector<uint32_t> fileRefs;
        fileRefs.reserve(freqWordFile.size());

        std::unordered_map<size_t, std::vector<PostingTask>> chunkMap;
        chunkMap.reserve(freqWordFile.size());

        for (auto& [word, count] : freqWordFile)
        {
            uint32_t wid = wordIds.getId(word);
            fileRefs.push_back(wid);

            size_t chunkId = wid / CHUNK_SIZE;
            chunkMap[chunkId].push_back({wid, (uint32_t)count});
        }

// === 2. Отправляем запись wordRefs в strand ===
        boost::asio::post(strand_,
                          [this, fileId, fileRefs = std::move(fileRefs)]() mutable
                          {
                              wordRefs[fileId] = std::move(fileRefs);
                          });

// === 3. Отправляем commit чанков в io_commit ===
        boost::asio::post(io_commit_,
                          [this,
                                  fileId,
                                  ...
                                  mutable
                          {
                              commitChunkMap(fileId, chunkMap);
        ...
        const std::unordered_map<size_t, std::vector<PostingTask>>& chunkMap)
{
    ...
    for (auto& [cid, tasks] : chunkMap)
    {
        ...
        for (auto& t : tasks)
        {
            ...
            ch.bucket[local][fileId] += t.count;
        }
    }
}
...
std::future<void> inverted_index::InvertedIndex::updateDocumentBase(
        const std::list<std::wstring>& vecPaths)
{
    ...
    boost::asio::post(
                    ...
                    finalPromise]() mutable
            {
                ...
                for (FileId fileId : toIndex)
                {
                    ...
                    fileFutures.push_back(p->get_future());

                    // чтение/парсинг файла — в io_
                    boost::asio::post(
                            io_,
                            [this, fileId, p]()
                            {
                                try {
                                    fileIndexing(fileId, p); // внутри создаёт batch и вызывает applyBatchInStrand
                                }
                                catch (...)
                                {
                                    try {
                ...
                std::thread(
                                ...
                                finalPromise]() mutable
                        {
                            ...
                            for (auto& fut : fileFutures)
                            {
                                ...
                                {
                                    addToLog("fileIndexingException in updateDocumentBase");
                                }
                            }
                            ...
                            boost::asio::post(
                                    ...
                                    [this, finalPromise]()
                                    {
                                        addToLog("finalPromise ....");
                                        // при желании здесь:
                                        // compact();
                                        saveIndex();
                                        work = false;
                                        finalPromise->set_value();
                                        addToLog("finalPromise set done!!");
                                    });
                        }).detach();
            });
...
void inverted_index::InvertedIndex::addToLog(const string& _s) const {
    ...
    logFile.open("log"+ std::to_string(tid)  +".ini", ios::app);
...
inverted_index::InvertedIndex::InvertedIndex(boost::asio::io_context& io)
        : io_(io)
        ...
        , io_commit_()
{
    ...
    commit_guard_ =
            std::make_unique<boost::asio::executor_work_guard<boost::asio::io_context::executor_type>>(
                    boost::asio::make_work_guard(io_commit_));

    // Запускаем 3 commit-потока
    for (int i = 0; i < 5; i++)
    {
