...
namespace auth
{
    ...
    void AuthClientStore::ensureSchema()
    {
        ...
        execOrThrow(
            ...
            "ON clients(client_id, client_name, flash_serial, enabled);");
        execOrThrow("PRAGMA user_version = 1;");
    }
        ...
        const std::string& signature_meta)
    {
        if (client_id.empty() || client_name.empty() || flash_serial.empty()) {
            throw std::invalid_argument(
                "client_id, client_name and flash_serial must be non-empty");
        }
        ...
        constexpr const char* sql =
            "INSERT INTO clients("
            "client_id, client_name, flash_serial, enabled, signature_meta, "
            "created_at, updated_at) "
            "VALUES(?1, ?2, ?3, ?4, ?5, ?6, ?7) "
            "ON CONFLICT(client_id) DO UPDATE SET "
            "client_name=excluded.client_name, "
            "flash_serial=excluded.flash_serial, "
        ...
        sqlite3_bind_text(statement, 3, flash_serial.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(statement, 4, enabled ? 1 : 0);
        sqlite3_bind_text(
            statement, 5, signature_meta.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int64(statement, 6, stamp);
        ...
        const std::string& client_id) const
    {
        ...
        sqlite3_stmt* statement = nullptr;
        constexpr const char* sql =
            "SELECT client_id, client_name, flash_serial, enabled, "
        ...
        const std::string& flash_serial) const
    {
        ...
        constexpr const char* sql =
            ...
            "signature_meta, created_at, updated_at "
            "FROM clients "
            "WHERE client_id = ?1 AND client_name = ?2 AND flash_serial = ?3 "
            "AND enabled = 1 "
            "LIMIT 1;";
        if (sqlite3_prepare_v2(db_, sql, -1, &statement, nullptr) != SQLITE_OK) {
        ...
        sqlite3_bind_text(statement, 2, client_name.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(
            statement, 3, flash_serial.c_str(), -1, SQLITE_TRANSIENT);
