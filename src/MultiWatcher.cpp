...
MultiDirWatcher::MultiDirWatcher(const std::wstring& p,
                                 DirMatch match,
                                 FileCb   fcb)
        : parentDir_(p), matchFn_(std::move(match)), fileCb_(std::move(fcb))
{}
...
void MultiDirWatcher::start()
{
    ...
    try {
                                       ...
                                       fs::directory_options::skip_permission_denied))
        {
            ...
            {
                auto w = std::make_unique<FileWatcher>(
            ...
            if (interesting)
            {
                ...
                switch (fni->Action)
                {
                    ...
                    case FILE_ACTION_RENAMED_NEW_NAME:
                    {
                        ...
                        {
                            auto w = std::make_unique<FileWatcher>(childPath, fileCb_, true /*forceWalk*/ );
                            w->start();
                            inner_.emplace(name, std::move(w));
