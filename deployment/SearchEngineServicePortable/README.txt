...
копию всего каталога установки, data-dir, индексируемых папок и месячных баз,
исправляет пути
...
повторно проверяются manifest, SHA-256, SQLite и оба полных дерева среды
выполнения. После успешной очистки целиком отсутствуют каталог установки
(bin, tools, документация и ServiceInstance.cmd) и data-dir вместе с логами и
SQLite sidecar-файлами -wal/-shm/-journal. При неполном удалении manifest не
получает успешную фазу; очистку можно безопасно повторить. Декабрьская база
года из
...
подтверждению полностью удаляет выбранную службу, firewall rule, фактические
каталоги программы/данных из SCM и остатки в стандартных путях. Для
замороженной службы удаляется весь активный каталог архива, а не только
program/data внутри него.
Также удаляются оставшиеся rollback-каталоги этого экземпляра. Каталоги
