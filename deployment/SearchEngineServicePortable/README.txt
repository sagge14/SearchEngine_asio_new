...
3. Проверьте в data\Settings.json каталоги config.dirs, exclude_dirs,
   prm_base_dir и prd_base_dir. Создайте нужные локальные каталоги.
4. У каждого экземпляра должен быть свой свободный TCP-порт.
5. Для сетевых ресурсов используйте UNC-пути, а не подключённые буквы дисков.
6. Запустите Install-SearchEngineService.bat от имени администратора.
...
«краткого содержания» AutoPad PRM и место хранения каталога документов.
...
старых данных. До успешного ответа сервера PING/PONG старые файлы сохраняются
для автоматического отката.
