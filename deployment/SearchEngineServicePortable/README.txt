...
- выпускает computer-токен client_id=local-machine, client_name=operator,
  сохраняет его в
  %LOCALAPPDATA%\SearchEngine\searchclient-auth-token.json и регистрирует в
...
При ошибке установщик отдельно указывает этап (UUID, выпуск токена, экспорт
открытого ключа или регистрация в SQLite), код завершения и путь к журналу в
%TEMP%. Для существующего хранилища также выводятся его путь и пояснение о
...
получает успешную фазу; очистку можно безопасно повторить. Архивная копия
декабрьской BASES_PRD (`12-<year>.db3`) создаётся всегда. Исходную декабрьскую
базу можно оставить для совместимости с TverdakManager; тогда она остаётся на
прежнем месте, но получает пути к телеграммам архивного сервера. При возврате в
исходные места эта база без отдельного вопроса заменяется восстановленной
копией с активными путями. Операция возврата также восстанавливает
`server_mode=active` и исходный SCM ImagePath, в том числе после подтверждённой
очистки старого места.
