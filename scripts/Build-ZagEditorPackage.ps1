...
foreach ($build in $builds) {
    ...
    try {
        ...
        Invoke-SearchEngineCmakeBuildWithoutVersionBump {
            & cmake --build --preset $build.Build --target ZagEditor -- /m
            if ($LASTEXITCODE -ne 0) {
                throw (
                    "ZagEditor $($build.Architecture) Release build failed with " +
    ...
    if (-not [string]::IsNullOrWhiteSpace($ConfigPath)) {
        $packageArguments.ConfigPath = $ConfigPath
    }
    if (-not [string]::IsNullOrWhiteSpace($DictPath)) {
        $packageArguments.DictPath = $DictPath
    }
    ...
    & (Join-Path $PSScriptRoot 'New-ZagEditorPackage.ps1') @packageArguments
}
